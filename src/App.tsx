...
function App() {
  ...
  useEffect(() => {
    ...
    const title = current ? current.seoTitle : route === '/pakistan' ? 'Pakistan Utility Guides | ToolNest' : route === '/about' ? 'About ToolNest' : 'ToolNest | Free Online Tools, PDF Tools & Pakistan Utility Guides'
    const description = current?.seoDescription || 'Free online tools for PDF, image, text, calculator, and developer tasks. Explore Pakistan utility guides for Telenor, Jazz, Zong, and Ufone.'
    ...
    const canonical = document.querySelector('link[rel="canonical"]'); if (canonical) canonical.setAttribute('href', `https://toolnest.example${route || '/'}`)
    ...
    const itemList = tools.slice(0, 8).map((tool, index) => ({
      ...
      item: {
        ...
        url: `https://toolnest.example/tools/${tool.slug}`
      }
    }))
    ...
    const schema = {
      ...
      '@graph': [
        { '@type': 'WebSite', name: 'ToolNest', url: 'https://toolnest.example/' },
        {
          '@type': current ? 'WebPage' : 'CollectionPage',
          name: title,
          url: `https://toolnest.example${route || '/'}`,
          description: description,
          isPartOf: { '@type': 'WebSite', name: 'ToolNest', url: 'https://toolnest.example/' },
